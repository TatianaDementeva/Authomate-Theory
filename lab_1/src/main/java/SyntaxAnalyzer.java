...
public class SyntaxAnalyzer {
    ...
    private static HashSet<CurrentCondition> step(Deque<Transition> nfa,HashSet<CurrentCondition> currentTreeLevel){

        HashSet<CurrentCondition> nextTreeLevel = new HashSet<>();

        for (CurrentCondition cc: currentTreeLevel) {
            if(cc.getS().length() == 0){continue;}
            char currentChar = cc.getS().charAt(0);

            for (Transition p:nfa) {
